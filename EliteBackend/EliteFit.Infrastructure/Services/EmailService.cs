...
namespace EliteFit.Infrastructure.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<string?> SendPasswordResetEmailAsync(string toEmail, string toName, string resetLink)
        {
            // Always log for dev visibility
            _logger.LogInformation("[EliteFit] Password reset link for {Email}: {Link}", toEmail, resetLink);

            if (!_settings.Enabled)
            {
                _logger.LogWarning(
                    "Email:Enabled = false — skipping SMTP. " +
                    "The reset link is returned in the API response for development use.");
                return resetLink;
            }
            ...
            catch (Exception ex)
            {
                ...
                // Don't rethrow — token is already saved; user can retry
            }
