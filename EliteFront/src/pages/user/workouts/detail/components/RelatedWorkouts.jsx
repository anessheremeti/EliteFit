    ...
    <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-black/5">
        {loading
          ...
          : items.map(w => {
              // Llogaritja e minutave nga sekondat e backend-it (durationSeconds)
              const durationMin = w.durationSeconds 
                ? Math.round(w.durationSeconds / 60) 
                : 0;

              return (
                <Link
                  key={w.id} // w.id në camelCase saktë nga API
                ...
                >
                  ...
                  <div className="w-20 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0">
                    <img
                      src={w.thumbnailUrl || w.thumbnail || PLACEHOLDER}
                      alt={w.title}
